...
#
# Writes: dist/Mega-Man-Legends-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix Mega-Man-Legends-Recomp \
  --exe-name Mega_Man_Legends_Recompiled \
  --display-name "Mega Man Legends Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Mega Man Legends disc" \
